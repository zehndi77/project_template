...
progressstep = 1/(numberruns*11*11); % setup for progressbar
progress = 0; 
...
ratiosave = zeros(11,11,numberruns);
%% ########### Main Loop ###########
for n = 1:numberruns
    sizenr = 1;
    betanr = 1;
    for pc = 0:0.1:1
        for p_pc = 0:0.1:1
            ...
            [statssave,statssave_no_pirate,graphm,stats,ratio] = masterpirate(size,beta,tmax,delay,decayinit,pc,p_pc);
            %% Saving ratio
            ratiosave(sizenr,betanr,n) = ratio; % save ratio for plot;
            %% Saving Stats
            % set up name
            name = {mat2str(pc),mat2str(p_pc),mat2str(tmax),mat2str(n),'.mat'}; %##
